...
void UPathfindingSubsystem::PopulateNodes()
{
	...
	{
		Nodes.Add(*It);
		NodePositions.Add((*It)->GetActorLocation());
...
TArray<FVector> UPathfindingSubsystem::ReconstructPath(const TMap<ANavigationNode*, ANavigationNode*>& CameFromMap, ANavigationNode* EndNode)
{
	...
	TArray<FVector> ReverseNodeLocations;
	...
	const int32 NodesNum = NodeLocations.Num();
	while(!NodeLocations.IsEmpty())
	{
		ReverseNodeLocations.Add(NodeLocations.Pop());
	}
